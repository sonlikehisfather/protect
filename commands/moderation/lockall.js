  ...
  async run(client, message) {
    ...
    const confirmMessage = await message.channel.send({
      embeds: [
        embed.build(
          guildId,
          'Es-tu sûr de vouloir verrouiller TOUS les salons du serveur ? Cette action est irréversible.',
          {
            title     : 'Confirmer lockall',
            timestamp : false,
          }
        )
      ],
    ...
    collector.on('collect', async interaction => {
      if (interaction.customId === 'local:lockall:cancel') {
        ...
        await interaction.update({
          embeds: [
            embed.build(
              guildId,
              'Action annulée.',
              { title: 'Lockall annulé', timestamp: false }
            )
          ],
      ...
      if (interaction.customId !== 'local:lockall:confirm') {
      ...
      await _runLockAll(message, guild, guildId, deleteReply, deleteDelay);
    });
...
async function _runLockAll(message, guild, guildId, deleteReply, deleteDelay) {
  ...
  for (const channel of guild.channels.cache.values()) {
    ...
    const alreadyLocked = isTextChannel(channel)
...
function _buildConfirmRow(disabled) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('local:lockall:confirm')
      .setLabel('Confirmer')
      .setStyle(ButtonStyle.Danger)
      .setDisabled(disabled),
    new ButtonBuilder()
      .setCustomId('local:lockall:cancel')
      .setLabel('Annuler')
