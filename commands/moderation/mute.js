...
module.exports = {
  ...
  async run(client, message, args) {
    ...
    if (!perms.check(message, 'mute')) return;
    ...
    if (!me.permissions.has('ModerateMembers') &&
      ...
      const sent = await embed.replyError(
        message,
        'Je n’ai pas les permissions nécessaires pour mute.',
        { timestamp: false }
      ).catch(() => null);
    ...
    if (args[1]) {
      ...
      durationStr = args[1];
      durationMs  = parseDuration(durationStr, { minMs: 1000 });
    ...
    const reason = args.slice(durationStr ? 2 : 1).join(' ')
    ...
    if (useTimeout) {
      ...
      if (!durationMs) {
        const sent = await embed.replyError(
          message,
          'Le timeout Discord nécessite une durée. Exemple : `10m`, `1h`, `1d`.',
          { timestamp: false }
        ).catch(() => null);
      ...
      if (durationMs > 28 * 24 * 60 * 60 * 1000) {
        const sent = await embed.replyError(
          message,
          'Le timeout Discord ne peut pas dépasser 28 jours.',
          { timestamp: false }
        ).catch(() => null);
      ...
      const muted = await member.timeout(
        durationMs ?? null,
        reason
      ...
      const muted = await member.roles.add(muteRole, reason).then(() => true).catch(() => false);
      ...
      if (durationMs && typeof db.addTempRole === 'function') {
        db.addTempRole(
          guildId,
          member.id,
          muteRoleId,
    ...
    db.addSanction(
      ...
      'mute',
      reason,
      durationMs ? Math.floor(durationMs / 1000) : null
    );
