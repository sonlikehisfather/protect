  ...
  async run(client, message) {
    ...
    const confirmMessage = await message.channel.send({
      embeds: [
        embed.build(
          guildId,
          'Es-tu sûr de vouloir afficher TOUS les salons du serveur ? Cette action est irréversible.',
          {
            title     : 'Confirmer unhideall',
            timestamp : false,
          }
        )
      ],
    ...
    collector.on('collect', async interaction => {
      if (interaction.customId === 'local:unhideall:cancel') {
        ...
        await interaction.update({
          embeds: [
            embed.build(
              guildId,
              'Action annulée.',
              { title: 'Unhideall annulé', timestamp: false }
            )
          ],
      ...
      if (interaction.customId !== 'local:unhideall:confirm') {
      ...
      await _runUnhideAll(message, guild, guildId, deleteReply, deleteDelay);
    });
...
async function _runUnhideAll(message, guild, guildId, deleteReply, deleteDelay) {
  ...
  for (const channel of guild.channels.cache.values()) {
    ...
    const edited = await channel.permissionOverwrites
      .edit(everyone, { ViewChannel: null })
      .catch(() => null);

    if (edited) unhiddenCount++;
...
function _buildConfirmRow(disabled) {
  return new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      .setCustomId('local:unhideall:confirm')
      .setLabel('Confirmer')
      .setStyle(ButtonStyle.Danger)
      .setDisabled(disabled),
    new ButtonBuilder()
      .setCustomId('local:unhideall:cancel')
      .setLabel('Annuler')
