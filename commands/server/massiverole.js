...
module.exports = {
  help: {
    name        : 'massiverole',
    description : 'Ajoute un rôle à tous les membres ou aux membres ayant un rôle précis.',
    usage       : 'massiverole [rôle source] <rôle à ajouter>',
    aliases     : [],
  },
  ...
  async run(client, message, args) {
    ...
    if (!perms.check(message, 'massiverole')) return;
    ...
    if (!args.length) {
      const sent = await embed.replyError(
        message,
        `Utilisation : \`${message.prefix || '+'}massiverole [rôle source] <rôle à ajouter>\``,
        { timestamp: false }
      ).catch(() => null);
    ...
    if (targetRole.id === guild.id) {
      const sent = await embed.replyError(
        message,
        'Vous ne pouvez pas ajouter le rôle everyone.',
        { timestamp: false }
      ).catch(() => null);
    ...
    if (targetRole.managed) {
      const sent = await embed.replyError(
        message,
        'Vous ne pouvez pas ajouter un rôle géré par une intégration.',
        { timestamp: false }
      ).catch(() => null);
    ...
    if (targetRole.position >= me.roles.highest.position) {
      const sent = await embed.replyError(
        message,
        'Je ne peux pas ajouter ce rôle car il est supérieur ou égal à mon rôle le plus haut.',
        { timestamp: false }
      ).catch(() => null);
    ...
    if (
      ...
      const sent = await embed.replyError(
        message,
        'Vous ne pouvez pas attribuer un rôle supérieur ou égal à votre rôle le plus haut.',
        { timestamp: false }
      ).catch(() => null);
    ...
    const confirmMessage = await message.channel.send({
      embeds: [
        embed.build(
          ...
          confirmText,
          {
            title    : 'Confirmer massiverole',
            timestamp: false,
          }
        ),
      ],
      components      : [_buildConfirmRow('massiverole', false)],
    ...
    collector.on('collect', async interaction => {
      if (interaction.customId === 'local:massiverole:cancel') {
        ...
        await interaction.update({
          embeds: [
            embed.build(
              guildId,
              'Action annulée.',
              { title: 'Massiverole annulé', timestamp: false }
            ),
          ],
          components: [_buildConfirmRow('massiverole', true)],
        }).catch(() => {});
        return;
      }

      if (interaction.customId !== 'local:massiverole:confirm') {
...
async function _runMassiveRole(message, guild, guildId, members, sourceRole, targetRole, protectedCount, deleteReply, deleteDelay) {
  ...
  for (const member of members.values()) {
    const ok = await member.roles.add(
      targetRole,
      `Massiverole par ${message.author.username}`
